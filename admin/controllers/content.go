...
func (this *ContentController) RenderTemplate(w http.ResponseWriter, name string, data *models.Content) error {
    // Ensure the template exists in the map.
    tmpl, ok := helpers.Templates[name]
    if !ok {
        return fmt.Errorf("The template %s does not exist.", name)
    }
    fmt.Print(helpers.Templates)
...
func (this *ContentController) Post(w http.ResponseWriter, r *http.Request) {
...
func (this *ContentController) PutContent(w http.ResponseWriter, r *http.Request) {
    w.Header().Set("Content-Type", "application/json")
...
func (this *ContentController) RenderContent(w http.ResponseWriter, r *http.Request) {
    ...
    if(strings.Contains(myToken, "Bearer ")){
        myToken = strings.Replace(myToken, "Bearer ", "", -1)
        fmt.Println("\nToken: " + myToken)
    }

    token, err := helpers.GetToken(myToken)

    if(err == nil){
        fmt.Println("\nAWESOME TOKEN APPROVED1!\n")
        fmt.Println(token)
        content.Token = token
    }else{
        fmt.Println(err.Error())
        // helpers.Templates["Unauthorized.tmpl"] = template.Must(template.ParseFiles("views/Unauthorized.tmpl"))
        // this.RenderTemplate(w, "Unauthorized.tmpl", nil)
        //fmt.Fprintf(w,"%s","<h2>Unauthorized Access</h2><p>You need to login first, in order to access any content on this website</p>")
    }
    ...
    // if(token.Valid){
        ...
        if(templateName !=".tmpl"){
            if(helpers.Templates[templateName] != nil){
                ...
                if(content.Template.Node.ParentNodes != nil || content.Template.PartialTemplates != nil){
                    ...
                    // TODO - MAKE nodes array inside each parent node and reference the node name instead, since files follow not alias but node name
                    ...
                    // if(content.Template.PartialTemplates != nil){
                    //     partialTemplates := content.Template.PartialTemplates

                    //     x := make([]string, 0, len(partialTemplates))

                    //     for  _, value := range partialTemplates {
                    //         tplName := "views/" + value.Name + ".tmpl"
                    //         x = append(x, tplName)
                    //     }
                    ...
                    helpers.Templates[templateName] = template.Must(template.ParseFiles(templateArray...))
                    //.Delims("{@","@}")
                } else {
                    helpers.Templates[templateName] = template.Must(template.ParseFiles("views/" + templateName))
                }
            }
            fmt.Println("CONTENT.Token::: ")
            fmt.Println(content.Token)
            fmt.Println("!_- CONTENT.token end")
            this.RenderTemplate(w, templateName, &content)
        }
    // } else{
    //     helpers.Templates["Unauthorized.tmpl"] = template.Must(template.ParseFiles("views/Unauthorized.tmpl", "views/Layout.tmpl"))
    //     this.RenderTemplate(w, "views/Unauthorized.tmpl", nil)
    // }
...
func (this *ContentController) GetBackendContentByNodeId(w http.ResponseWriter, r *http.Request) {
    w.Header().Set("Content-Type", "application/json")
    ...
    content := models.GetBackendContentByNodeId(id)
...
// func (this *ContentController) Get(w http.ResponseWriter, r *http.Request) {
//     w.Header().Set("Content-Type", "application/json")
...
// func (this *ContentController) GetContentOfType(w http.ResponseWriter, r *http.Request) {
//     w.Header().Set("Content-Type", "application/json")
